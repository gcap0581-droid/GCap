...
export function formatDatabaseToCentralResponse(
  ...
  if (role === 'ADMIN') {
    ...
    const { aliases: msgAliases } = userId
      ? findUserAndAllAliases(userId, fs.users || [])
      : { aliases: [] };
    const lowerAliases = msgAliases.map((a) => a.toLowerCase());

    const hasInvestmentsForMsg = (fs.investments || []).some((inv: any) => {
      if (inv.status !== 'ACTIVE') return false;
      const iUid = (inv.userId || '').toLowerCase();
      return lowerAliases.includes(iUid);
    });

    const userWalletForMsg = userId ? getWalletForUser(userId, fs.wallets || {}, fs.users || []) : null;
    const hasPositiveBalanceForMsg = userWalletForMsg
      ? (userWalletForMsg.cashBalance || 0) > 0 || (userWalletForMsg.gpBalance || 0) > 0
      : false;

    const userMsgs = (fs.messages || []).filter((m: any) => {
      if (!m) return false;
      ...
      if (m.targetType === 'SINGLE') {
        ...
        return lowerAliases.includes(tId) || lowerAliases.includes(tLogin);
      }
      if (m.targetType === 'INVESTORS') return hasInvestmentsForMsg;
      if (m.targetType === 'POSITIVE_BALANCE') return hasPositiveBalanceForMsg;
      if (m.targetType === 'SELECTED') {
        if (!Array.isArray(m.targetUserIds)) return false;
        const selectedList = m.targetUserIds.map((id: any) => String(id).toLowerCase());
        return lowerAliases.some((alias) => selectedList.includes(alias));
      }
      return false;
    });
    ...
    if (res.ok) {
      ...
      if (data && data.success) {
        if (Array.isArray((data as any).deletedUserIds)) {
          (data as any).deletedUserIds.forEach((id: string) => recordDeletedUserId(id));
        }
        if (data.users && Array.isArray(data.users)) {
          data.users = data.users.filter((u: any) => u && u.id && !isUserDeleted(u));
        }
...
export async function apiFetchMessages(
  ...
  try {
    ...
    const isReqAdmin = role === 'ADMIN' || String(role).toUpperCase() === 'ADMIN';
    if (isReqAdmin) {
      return { success: true, messages: allMsgs };
    } else {
      const { aliases } = userId ? findUserAndAllAliases(userId, fs?.users || []) : { aliases: [] };
      ...
      const filtered = allMsgs.filter((m) => {
        ...
        if (userId && (m.targetUserId === userId || (Array.isArray(m.targetUserIds) && m.targetUserIds.includes(userId)))) return true;
        return false;
      });
...
export async function apiSendAdminMessage(
  ...
  try {
    ...
    try {
      ...
      const updated = [newMsg, ...currentMsgs.filter((m: any) => m.id !== newMsg.id)];
...
/**
 * Sync historical messages batch to the server
 */
export async function apiSyncAdminMessagesBatch(
  messagesList: AdminMessage[]
): Promise<{ success: boolean; syncedCount?: number; totalMessages?: number; error?: string }> {
  try {
    const res = await apiFetch(`${API_BASE}/admin/messages/sync-batch`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    console.warn('[apiSyncAdminMessagesBatch] API failed, syncing directly to Firestore:', err);
  }
  ...
  try {
    ...
    // Dedupe and merge with local list
    ...
    await saveMessagesToFirestore(merged).catch(() => {});
    return { success: true, syncedCount: added, totalMessages: merged.length };
  } catch (fsErr: any) {
    return { success: false, error: fsErr?.message || 'Failed to sync messages' };
  }
}
