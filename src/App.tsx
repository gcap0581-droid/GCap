...
export default function App() {
  ...
  // Active User Live Presence Heartbeat & Responsive Presence Loop:
  // Instantly marks online when user is viewing or touching the app, with robust multi-layer pings
  useEffect(() => {
    if (!currentUser || !currentUser.id) return;

    const markOnline = () => {
      // 1. Ping Express backend /api/auth/heartbeat
      ...
      // 2. Update client-side Firestore presence document
      updatePresenceInFirestore(currentUser, true).catch(() => {});
    };

    // Mark online immediately on mount
    markOnline();

    // Repeat active heartbeat every 2.5 seconds while window is open
    ...
    // Refresh presence on tab focus / touch / interaction
    ...
    window.addEventListener('touchstart', handleForeground, { passive: true });
    window.addEventListener('click', handleForeground, { passive: true });

    return () => {
      clearInterval(heartbeatInterval);
      window.removeEventListener('focus', handleForeground);
      window.removeEventListener('touchstart', handleForeground);
      window.removeEventListener('click', handleForeground);
    };
  }, [currentUser?.id]);
  ...
  const [messages, setMessages] = useState<AdminMessage[]>(() => {
    try {
      const raw = localStorage.getItem('gcap_admin_messages_backup');
      if (raw) {
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed) && parsed.length > 0) return dedupeAdminMessages(parsed);
      }
    } catch {}
    return [];
  });

  // Save messages to local storage backup whenever updated
  useEffect(() => {
    if (Array.isArray(messages) && messages.length > 0) {
      try {
        localStorage.setItem('gcap_admin_messages_backup', JSON.stringify(messages));
      } catch {}
    }
  }, [messages]);
  ...
  useEffect(() => {
    ...
    const syncAppState = async () => {
      try {
        ...
        if (!isCancelled && state && state.success) {
        ...
        if (!isCancelled) {
          // Absolute mathematical time reconciliation on launch & resume:
          // Runs unconditionally whether online, offline, on Vercel, or mobile PWA
          try {
            const currentInvs = getStoredInvestments();
            ...
            if (res.hasChanges) {
              ...
              if (currentUser) {
                ...
                const delta = res.walletDeltas[userKey] || res.walletDeltas[currentUser.loginId] || (cleanPhone10 ? res.walletDeltas[cleanPhone10] : null);
                if (delta && (delta.totalEarnedDelta > 0 || delta.royaltyEarnedDelta > 0)) {
                  setWallet((prev) => {
                    if (!prev) return prev;
                    const updatedWallet = {
                      ...prev,
            ...
            console.warn('[StateSync] Reconcile error:', recErr);
          }
        }
      }
    };
  ...
  useEffect(() => {
    const unsubscribeFirestore = subscribeToFirestoreState((fs) => {
      ...
      if (currentUser?.role === 'ADMIN') {
        ...
        // Regular User
        if (fs.bankDetails && fs.bankDetails[currentUser.id]) {
          const myBank = fs.bankDetails[currentUser.id];
          currentUser.bankDetails = myBank;
          setStoredBankDetails(currentUser.id, myBank);
        }
        ...
        if (fs.messages && Array.isArray(fs.messages)) {
          const myMsgs = fs.messages.filter((m) => {
            if (!m) return false;
            if (m.targetType === 'ALL' || !m.targetType || m.targetUserId === 'ALL') return true;
            if (m.targetUserId === currentUser.id || (Array.isArray(m.targetUserIds) && m.targetUserIds.includes(currentUser.id))) return true;
            return isMessageForCurrentUser(m);
          });
          setMessages(dedupeAdminMessages(myMsgs));
        }
      }
    });
      ...
      try {
        ...
        if (currentUser.role === 'ADMIN') {
          ...
          if (state.bankDetails) {
            const bDetails = state.bankDetails as BankAccountDetails;
            currentUser.bankDetails = bDetails;
            setStoredBankDetails(currentUser.id, bDetails);
          }
          if (state.messages && Array.isArray(state.messages)) {
            setMessages((prev) => dedupeAdminMessages(state.messages));
          }
        }
      } catch {
  ...
  const isMessageForCurrentUser = useCallback((msg: AdminMessage) => {
    if (!msg) return false;
    // Admins send messages to users; Admin is never the intended recipient of admin announcements
    if (currentUser?.role === 'ADMIN') return false;

    // Broadcast for everyone
    if (msg.targetType === 'ALL' || !msg.targetType || msg.targetUserId === 'ALL') return true;

    // If user is not logged in, only ALL messages apply
    if (!currentUser) return false;

    const uId = String(currentUser.id || '').toLowerCase();
    const uLogin = String(currentUser.loginId || '').toLowerCase();
    const uPhone = String(currentUser.phone || '').toLowerCase();
    const uDigits = uPhone.replace(/[^0-9]/g, '');
    const uLast10 = uDigits.length >= 10 ? uDigits.slice(-10) : uDigits;

    const aliasList = [uId, uLogin, uPhone, uDigits, uLast10].filter(Boolean);

    if (msg.targetType === 'SINGLE') {
      const tId = String(msg.targetUserId || '').toLowerCase();
      const tLogin = String(msg.targetUserLoginId || '').toLowerCase();
      const tDigits = tId.replace(/[^0-9]/g, '');
      const tLast10 = tDigits.length >= 10 ? tDigits.slice(-10) : tDigits;

      const directMatch = aliasList.some(
        (a) => a === tId || a === tLogin || (a.startsWith('usr-') && a.replace('usr-', '') === tId)
      );
      const phoneMatch = Boolean(uLast10 && tLast10 && uLast10 === tLast10);
      return directMatch || phoneMatch;
    }
    if (msg.targetType === 'SELECTED') {
      if (!Array.isArray(msg.targetUserIds)) return false;
      const targetIds = msg.targetUserIds.map((s) => String(s).toLowerCase());
      return aliasList.some((alias) =>
        targetIds.some((t) => t === alias || (uLast10 && t.includes(uLast10)))
      );
    }
    ...
    if (msg.targetType === 'POSITIVE_BALANCE') {
      return (wallet?.cashBalance || 0) > 0 || (wallet?.gpBalance || 0) > 0 || (wallet?.totalEarned || 0) > 0;
    }
    return true;
  }, [currentUser, investments.length, wallet?.cashBalance, wallet?.gpBalance, wallet?.totalEarned]);
  ...
  const activePopupMessageRef = useRef(activePopupMessage);
  useEffect(() => {
    activePopupMessageRef.current = activePopupMessage;
  }, [activePopupMessage]);

  // Real-time messages fetch and synchronization
  const refreshMessages = useCallback(async () => {
    try {
      const isReqAdmin = currentUser?.role === 'ADMIN' || (desktopTab as string) === 'admin' || String(desktopTab).startsWith('admin');
      const reqRole = isReqAdmin ? 'ADMIN' : (currentUser?.role || 'USER');
      const res = await apiFetchMessages(currentUser?.id, reqRole as any);
      if (res && Array.isArray(res.messages)) {
        const uniqueList = dedupeAdminMessages(res.messages);
        if (currentUser?.role === 'ADMIN') {
          setMessages((prev) => dedupeAdminMessages([...uniqueList, ...prev]));
        } else {
        ...
        // Auto-popup latest unread broadcast message when opening app if not dismissed
        const dismissedList: string[] = currentUser?.dismissedPopupMsgIds || [];

        const unreadMsgs = uniqueList.filter((m) => {
          if (!m || !m.id) return false;
          if (m.showPopup === false) return false;
          if (currentUser && !isMessageForCurrentUser(m)) return false;

          const isLocalDismissed = dismissedList.includes(m.id);
          const isUserDismissed = currentUser && Array.isArray(m.dismissedByUserIds) && m.dismissedByUserIds.includes(currentUser.id);
          if (isLocalDismissed || isUserDismissed) return false;

          const isRead = currentUser && Array.isArray(m.readByUserIds) && m.readByUserIds.includes(currentUser.id);
          return !isRead;
        });

        if (unreadMsgs.length > 0 && !activePopupMessageRef.current) {
  ...
  }, [currentUser?.id, currentUser?.role, desktopTab]);
      ...
      try {
        ...
        if (raw) {
          ...
          if (Array.isArray(parsed) && parsed.length > 0) {
            hasSyncedHistoricalMessages.current = true;
            console.log('[GCap Sync] Detected local admin message backup. Syncing historical messages to server...', parsed.length);
            apiSyncAdminMessagesBatch(parsed)
              .then((syncRes) => {
                if (syncRes && syncRes.success) {
                  console.log('[GCap Sync] Successfully restored historical messages to central database!', syncRes.syncedCount);
                  // Refresh messages to reflect changes
                  refreshMessages();
                }
              })
              .catch((err) => {
                console.warn('[GCap Sync] Failed to sync historical messages batch:', err);
              });
          }
        }
      } catch (e) {
        console.warn('[GCap Sync] Error loading historical messages for sync:', e);
      }
    }
  ...
  useEffect(() => {
    const unsub = subscribeToRealtimeEvents((event) => {
      const isMsgEvent =
        event.type === 'ADMIN_MESSAGE' ||
        (event.type === 'STATE_CHANGED' && (event as any).type === 'MESSAGE_CREATED');

      if (isMsgEvent) {
        const payloadMsg: AdminMessage | undefined =
          event.adminMessage ||
          (event as any).message ||
          ((event as any).id && (event as any).title ? (event as any) : undefined);

        if (payloadMsg && payloadMsg.id) {
          setMessages((prev) => {
            const exists = prev.some((m) => m.id === payloadMsg.id);
            const list = exists ? prev.map((m) => (m.id === payloadMsg.id ? payloadMsg : m)) : [payloadMsg, ...prev];
            return dedupeAdminMessages(list);
          });

          // Show immediate popup modal if message is targeted to current user (or broadcast)
          const isTargeted = isMessageForCurrentUser(payloadMsg);
          if (isTargeted) {
            const dismissedList = currentUser?.dismissedPopupMsgIds || [];
            const isDismissed =
              dismissedList.includes(payloadMsg.id) ||
              (currentUser &&
                Array.isArray(payloadMsg.dismissedByUserIds) &&
                payloadMsg.dismissedByUserIds.includes(currentUser.id));
            ...
            if (!isDismissed && payloadMsg.showPopup !== false) {
  ...
  // Ensure unread messages trigger popup as soon as messages state is populated
  useEffect(() => {
    if (messages.length === 0 || activePopupMessage) return;
    ...
    const unread = messages.find((m) => {
      ...
      const isDismissed = dismissedList.includes(m.id) || (currentUser && Array.isArray(m.dismissedByUserIds) && m.dismissedByUserIds.includes(currentUser.id));
  ...
  const userVisibleMessages = useMemo(() => {
    if (currentUser?.role === 'ADMIN') {
      // Admin Notifications: Show pending user deposit & withdrawal transaction alerts
      const adminAlerts: AdminMessage[] = [];
      ...
      for (const d of pendingDeposits) {
        ...
        adminAlerts.push({
          ...
          contentHi: `यूज़र ${d.userName || d.userId || 'यूज़र'} ने ₹${d.amount.toLocaleString()} जमा (Deposit) हेतु अनुरोध भेजा है। UTR/संदर्भ: ${refStr}।`,
          createdAt: d.date || new Date().toISOString(),
          timestamp: new Date(d.date || Date.now()).getTime(),
          category: 'ALERT',
          priority: 'URGENT',
          targetType: 'ALL',
          showPopup: false,
      ...
      for (const w of pendingWithdrawals) {
        ...
        adminAlerts.push({
          ...
          contentHi: `यूज़र ${w.userName || w.userId || 'यूज़र'} ने ₹${w.amount.toLocaleString()} निकासी (Withdrawal) का अनुरोध भेजा है। विवरण: ${destStr}।`,
      ...
      return adminAlerts.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
    }
  ...
  const handleSendAdminMessage = async (msg: Partial<AdminMessage>): Promise<boolean> => {
    try {
      ...
      if (res.success && res.message) {
        setMessages((prev) => dedupeAdminMessages([res.message!, ...prev]));
        showToast(
  ...
  const handleDismissPopupMessage = async () => {
    if (activePopupMessage) {
      ...
      if (currentUser) {
        ...
        if (!currentList.includes(msgId)) {
          const updatedList = [...currentList, msgId];
  ...
  const handleDismissCongratulations = (invId?: string) => {
    ...
    if (id) {
      if (currentUser) {
        const currentList = currentUser.shownLockCongratsIds || [];
  ...
  useEffect(() => {
    const runCycleReconciliation = () => {
      ...
      if (res.hasChanges) {
        ...
        // Check if currentUser's wallet needs update
        if (currentUser) {
          const userKey = currentUser.id;
          const cleanPhone10 = currentUser.phone ? currentUser.phone.replace(/[^0-9]/g, '').slice(-10) : '';
          const delta = res.walletDeltas[userKey] || res.walletDeltas[currentUser.loginId] || (cleanPhone10 ? res.walletDeltas[cleanPhone10] : null);
          if (delta && (delta.totalEarnedDelta > 0 || delta.royaltyEarnedDelta > 0)) {
            ...
            // If referral enabled, compute team bonus for sponsor
            if (rules?.isReferralEnabled !== false) {
              ...
              if (l1SponsorCode) {
                ...
                if (l1User && l1User.id !== currentUser.id && (rules?.referralL1Percent || 0) > 0) {
                  const l1Bonus = Math.round(((delta.totalEarnedDelta * (rules?.referralL1Percent || 0)) / 100) * 100) / 100;
                  if (l1Bonus > 0) {
                    res.newTransactions.push({
            ...
            const brandNewCycleTxns = res.newTransactions.filter(t => t && t.id && !shownToastTxnIdsRef.current.has(t.id));
            if (brandNewCycleTxns.length > 0) {
              brandNewCycleTxns.forEach(t => shownToastTxnIdsRef.current.add(t.id));
              showToast(
                isHi ? '💰 6 घंटे की अर्निंग स्वतः जमा हुई!' : '💰 6-Hour Earning Auto-Credited!',
                isHi
                  ? `+${formatINR(delta.totalEarnedDelta)} स्वतः आपकी 'Total Earning' में जमा हो गए हैं।`
                  : `+${formatINR(delta.totalEarnedDelta)} automatically credited to your Total Earning.`
              );
            }
          }
        }
          ...
          {userVisibleMessages.length > 0 && (
            ...
            >
              <div className="flex items-center gap-3.5 min-w-0">
                <div className="w-11 h-11 rounded-2xl bg-amber-500/20 border border-amber-500/40 text-amber-400 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform shadow-inner">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 selection:bg-emerald-500 selection:text-white overflow-x-hidden w-full max-w-full">
                    ...
                    {userVisibleMessages.length > 0 && (
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0 flex-1">
                          <div className="w-9 h-9 rounded-xl bg-amber-500/20 border border-amber-500/40 text-amber-300 flex items-center justify-center shrink-0 shadow-inner">
