    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl">
      ...
      {filteredTransactions.length === 0 ? (
        ...
        <div className="space-y-4">
          {/* Mobile View: Premium Spacious Cards to avoid squishing */}
          <div className="sm:hidden space-y-3.5">
            {filteredTransactions.map((tx, idx) => {
              ...
              return (
                <div 
                  key={`${tx.id || 'tx'}-${idx}`} 
                  className="p-4 rounded-xl bg-slate-800/20 border border-slate-800/40 hover:bg-slate-800/30 transition-all flex flex-col gap-3"
                >
                  <div className="flex items-start justify-between gap-3">
                    {/* Left: Icon & Description details */}
                    <div className="flex gap-3">
                      <div className="w-9 h-9 rounded-xl bg-slate-800/80 border border-slate-700/30 flex items-center justify-center shrink-0 mt-0.5 shadow-sm text-slate-300">
                        {getTypeIcon(tx)}
                      </div>
                      <div className="space-y-1">
                        <div className="font-bold text-white text-[13px] flex flex-wrap items-center gap-1.5 leading-snug">
                          ...
                          {tx.type === 'WITHDRAWAL' && tx.withdrawalSource && (
                            ...
                            >
                              {tx.withdrawalSource === 'ROYALTY' ? 'Royalty' : 'Earning'}
                            </span>
                          )}
                          {tx.type === 'TRANSFER' && (
                            <span className="text-[9px] font-mono px-1 rounded font-black bg-indigo-500/20 text-indigo-300 border border-indigo-500/20">
                              GP
                            </span>
                          )}
                          {tx.type === 'SWAP_GP' && (
                            <span className="text-[9px] font-mono px-1 rounded font-black bg-teal-500/20 text-teal-300 border border-teal-500/20">
                              CASH➔GP
                            </span>
                          )}
                        </div>
                        ...
                        <div className="text-[11px] text-slate-500 font-mono space-y-0.5">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span>ID: {tx.referenceId}</span>
                            {tx.method && <span className="text-slate-600">• {tx.method}</span>}
                          </div>
                    ...
                    <div className="flex flex-col items-end gap-1.5 shrink-0 text-right">
                      {tx.type === 'TRANSFER' ? (
                        <span className={`font-mono font-black text-sm ${isReceivedTransfer ? 'text-emerald-400' : 'text-indigo-300'}`}>
                        ...
                        <span className={`font-mono font-black text-sm ${isPositive ? 'text-emerald-400' : 'text-slate-200'}`}>
                      ...
                      {tx.status === 'PENDING' && (
                        <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-amber-300 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20 animate-pulse">
                          <span>{isHi ? 'लंबित' : 'Pending'}</span>
                        </span>
                      )}
                      {tx.status === 'REJECTED' && (
                        <div className="flex flex-col items-end gap-1">
                          <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20">
                            <span>{isHi ? 'अस्वीकृत' : 'Rejected'}</span>
                          </span>
                          {(tx.rejectReason || tx.rejectReasonHi) && (
                            <div className="text-[10px] text-rose-300/90 leading-tight bg-rose-950/40 p-1 rounded border border-rose-900/40 max-w-[130px] text-right">
                              {isHi && tx.rejectReasonHi ? tx.rejectReasonHi : (tx.rejectReason || tx.rejectReasonHi)}
                            </div>
                          )}
                        </div>
                      )}
          ...
          {/* Desktop/Tablet Table View: Keep for wide screens with proper spacing */}
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-800/60 font-sans">
                {filteredTransactions.map((tx, idx) => {
                  ...
                  return (
                    <tr key={`${tx.id || 'tx'}-${idx}`} className="hover:bg-slate-800/30 transition-colors">
            ...
            <table className="w-full text-left text-[11px] border-collapse">
              ...
              <tbody className="divide-y divide-slate-200 font-mono text-xs">
                {transactions.map((tx, idx) => {
                  ...
                  return (
                    <tr key={`${tx.id || 'tx'}-${idx}`} className="text-slate-800">
