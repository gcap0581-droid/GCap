...
export const AdminMessagesTab: React.FC<AdminMessagesTabProps> = ({
  ...
  // Local message history state initialized from props with deduplication
  const dedupe = (list: AdminMessage[]) => {
  ...
  const [localMessages, setLocalMessages] = useState<AdminMessage[]>(() => {
    const fromProps = dedupe(messages);
    if (fromProps.length > 0) return fromProps;
    try {
      const raw = localStorage.getItem('gcap_admin_messages_backup');
      if (raw) {
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed) && parsed.length > 0) return dedupe(parsed);
      }
    } catch {}
    return [];
  });
  ...
  useEffect(() => {
    if (Array.isArray(messages) && messages.length > 0) {
      setLocalMessages((prev) => {
        const map = new Map<string, AdminMessage>();
        // Add existing prev messages first so locally created/resent messages aren't lost
        for (const m of prev || []) {
          if (m && m.id) map.set(m.id, m);
        }
        ...
        const updated = Array.from(map.values()).sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
        try {
          localStorage.setItem('gcap_admin_messages_backup', JSON.stringify(updated));
        } catch {}
        return updated;
      });
    }
  ...
  // Sync historical backup messages with the server
  const [unsyncedMessages, setUnsyncedMessages] = useState<AdminMessage[]>([]);
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          const serverIds = new Set(messages.map((m) => m && m.id).filter(Boolean));
          const unsynced = parsed.filter((m) => m && m.id && !serverIds.has(m.id));
          setUnsyncedMessages(unsynced);
        } else {
          setUnsyncedMessages([]);
        }
      } else {
  ...
  const handleSyncHistory = async () => {
    if (unsyncedMessages.length === 0 || isSyncingHistory) return;
    setIsSyncingHistory(true);
    try {
      const res = await apiSyncAdminMessagesBatch(unsyncedMessages);
      if (res.success) {
        setSuccessToast(
          isHi
            ? `🎉 ${res.syncedCount || unsyncedMessages.length} पुराने संदेशों का इतिहास सर्वर और डेटाबेस पर सफलतापूर्वक रीस्टोर हो गया है!`
            : `🎉 Successfully restored history of ${res.syncedCount || unsyncedMessages.length} previous messages to server & database!`
        );
  ...
  // Debounced Auto-Translate Content (English -> Hindi)
  const contentDebounceRef = useRef<NodeJS.Timeout | null>(null);
  useEffect(() => {
    if (!autoTranslate || !content.trim()) return;

    if (contentDebounceRef.current) clearTimeout(contentDebounceRef.current);
    contentDebounceRef.current = setTimeout(async () => {
      setIsTranslatingContent(true);
      try {
        const translated = await translateEnglishToHindi(content);
        if (translated) {
          setContentHi(translated);
        }
      } catch (err) {
        console.warn('Auto translate content error:', err);
      } finally {
  ...
  // Debounced Auto-Translate Title (English -> Hindi)
  const titleDebounceRef = useRef<NodeJS.Timeout | null>(null);
  useEffect(() => {
    if (!autoTranslate || !title.trim()) return;

    if (titleDebounceRef.current) clearTimeout(titleDebounceRef.current);
    titleDebounceRef.current = setTimeout(async () => {
      setIsTranslatingTitle(true);
      try {
        const translated = await translateEnglishToHindi(title);
        if (translated) {
          setTitleHi(translated);
        }
      } catch (err) {
        console.warn('Auto translate title error:', err);
      } finally {
  ...
  const handleManualTranslate = async () => {
    if (title.trim()) {
      setIsTranslatingTitle(true);
      try {
        const t = await translateEnglishToHindi(title);
        if (t) setTitleHi(t);
      } finally {
        setIsTranslatingTitle(false);
      }
    ...
    if (content.trim()) {
      setIsTranslatingContent(true);
      try {
        const c = await translateEnglishToHindi(content);
        if (c) setContentHi(c);
      } finally {
        setIsTranslatingContent(false);
      }
  ...
  const handleEditAndResend = (msg: AdminMessage) => {
    setEditingMessage(msg);
    setTargetType(msg.targetType || 'ALL');
    if (msg.targetType === 'SINGLE') {
      setSelectedUserId(msg.targetUserId || '');
    } else if (msg.targetType === 'SELECTED') {
      setSelectedUserIds(msg.targetUserIds || []);
    }
    setCategory(msg.category || 'ANNOUNCEMENT');
    setPriority(msg.priority === 'URGENT' || (msg.priority as string) === 'HIGH' || (msg.priority as string) === 'POPUP' ? 'URGENT' : 'NORMAL');
    setShowPopup(msg.showPopup ?? true);
    setTitle(msg.title || '');
    setTitleHi(msg.titleHi || msg.title || '');
    setContent(msg.content || '');
    setContentHi(msg.contentHi || msg.content || '');
    if (msg.senderName) setSenderName(msg.senderName);
  ...
  const handleResendDirect = async (msg: AdminMessage) => {
    if (isSubmitting) return;

    const finalSender = msg.senderName || senderName || 'GCap Official Admin';

    const newMsg: AdminMessage = {
      id: `msg-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
      title: msg.title,
      titleHi: msg.titleHi || msg.title,
      content: msg.content,
      contentHi: msg.contentHi || msg.content,
    ...
    setLocalMessages((prev) => [newMsg, ...prev.filter((m) => m.id !== newMsg.id)]);
    setIsSubmitting(true);

    try {
      await Promise.race([
        onSendMessage(newMsg),
        new Promise<boolean>((resolve) => setTimeout(() => resolve(true), 3500)),
      ]);

      setSuccessToast(
        isHi
          ? `संदेश "${msg.title}" दोबारा (Re-send) सफलतापूर्वक भेज दिया गया है!`
          : `Message "${msg.title}" re-sent successfully!`
      );
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    const finalTitleHi = titleHi.trim() || finalTitle;
    const finalContent = content.trim();
    const finalContentHi = contentHi.trim() || finalContent;
    const finalSender = senderName.trim() || 'GCap Official Admin';

    // Optimistically update local message list immediately
    const optimisticMessage: AdminMessage = {
      id: editingMessage ? editingMessage.id : `msg-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
      title: finalTitle,
      ...
      timestamp: Date.now(),
      readByUserIds: editingMessage ? (editingMessage.readByUserIds || []) : [],
      dismissedByUserIds: editingMessage ? (editingMessage.dismissedByUserIds || []) : [],
    };

    setLocalMessages((prev) => {
      const updated = [optimisticMessage, ...prev.filter((m) => m.id !== optimisticMessage.id)];
      try {
        localStorage.setItem('gcap_admin_messages_backup', JSON.stringify(updated));
      } catch {}
      return updated;
    ...
    try {
      ...
      setSuccessToast(
        isHi
          ? 'संदेश सफलतापूर्वक प्रसारित हो गया! सभी लक्षित यूज़र्स की स्क्रीन पर यह तुरंत दिखेगा।'
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200" id="admin-messages-tab">
      ...
      >
        {editingMessage && (
          <div className="p-3.5 rounded-xl bg-amber-500/15 border border-amber-500/40 flex items-center justify-between text-amber-300 text-xs shadow-md animate-in fade-in duration-200">
            <div className="flex items-center gap-2">
              <Edit3 className="w-4 h-4 text-amber-400 shrink-0" />
              <span>
                {isHi
                  ? 'पूर्व प्रसारित संदेश को एडिट करके पुनः भेजने की प्रक्रिया जारी है:'
                  : 'Editing & re-sending message:'}{' '}
                <strong className="text-white">"{editingMessage.title}"</strong>
              </span>
            </div>
            <button
              type="button"
              ...
              className="text-amber-400 hover:text-white underline text-[11px] font-bold cursor-pointer"
            >
              {isHi ? 'रद्द करें' : 'Cancel Edit'}
            </button>
          </div>
        )}
        ...
        {/* Auto-Translate Control & Status Bar */}
        <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-300">
              {isHi ? 'ऑटो हिंदी अनुवाद (Auto-Translate English ➔ Hindi):' : 'Auto-Translate (English ➔ Hindi):'}
            </span>
            <button
              type="button"
            ...
            >
              {autoTranslate ? (isHi ? '🟢 सक्रिय (ON)' : '🟢 Enabled (ON)') : (isHi ? '⚪ निष्क्रिय (OFF)' : '⚪ Disabled (OFF)')}
            </button>
          </div>

          <div className="flex items-center gap-2">
            {(isTranslatingTitle || isTranslatingContent) && (
              <span className="text-[11px] text-amber-400 flex items-center gap-1 animate-pulse">
                <Loader2 className="w-3 h-3 animate-spin" />
                <span>{isHi ? 'हिंदी अनुवाद हो रहा है...' : 'Translating to Hindi...'}</span>
              </span>
            )}
            <button
              type="button"
              onClick={handleManualTranslate}
              className="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-amber-300 border border-amber-500/30 font-bold text-[11px] flex items-center gap-1 cursor-pointer transition-colors"
              title="Click to translate English text into Hindi immediately"
            >
              <Sparkles className="w-3 h-3" />
              <span>{isHi ? 'अभी अनुवाद करें' : 'Translate Now'}</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                <span>{isHi ? 'संदेश शीर्षक (Title - हिंदी):' : 'Title (Hindi):'}</span>
                {isTranslatingTitle ? (
                  <Loader2 className="w-3 h-3 animate-spin text-amber-400" />
                ) : titleHi ? (
                  <span className="text-[10px] text-emerald-400 font-normal">✓ ऑटो-अनुवादित</span>
                ) : null}
              </label>
              <span className="text-[10px] text-amber-400/80 font-mono">हिंदी (देवनागरी)</span>
            </div>
            <input
              type="text"
              placeholder={isTranslatingTitle ? 'हिंदी में अनुवाद हो रहा है...' : 'उदा. मासिक निकासी विंडो और महत्वपूर्ण सूचना'}
              value={titleHi}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-slate-500">अंग्रेजी में लिखें (Type in English)</span>
            </div>
            <textarea
              required
              rows={4}
              placeholder="Write full message content in English here..."
              value={content}
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                <span>{isHi ? 'संदेश विवरण (Message Body - हिंदी):' : 'Message Body (Hindi):'}</span>
                {isTranslatingContent ? (
                  ...
                  <span className="text-[10px] text-emerald-400 font-normal">✓ स्वतः हिंदी अनुवाद तैयार</span>
                ) : null}
              </label>
              <span className="text-[10px] text-emerald-400/80 font-mono">Auto Hindi Body</span>
            </div>
            <textarea
              rows={4}
              placeholder={
                isTranslatingContent
                  ? 'अंग्रेजी से स्वतः हिंदी में अनुवाद हो रहा है...'
                  : 'यहाँ अंग्रेजी लिखने पर स्वतः हिंदी में अनुवाद होकर आ जाएगा...'
              }
              value={contentHi}
      ...
      <div className="p-4 sm:p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-4 shadow-xl" id="admin-message-history">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-400 font-mono">
            {isHi ? 'कुल संदेश:' : 'Total:'} <strong className="text-white">{localMessages.length}</strong>
          </span>
        </div>

        {/* Sync Local Storage backup with server/database */}
        {unsyncedMessages.length > 0 && (
          <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-pulse" id="sync-history-banner">
            <div className="space-y-1">
              <h4 className="text-xs sm:text-sm font-bold text-amber-300 flex items-center gap-1.5">
                <AlertTriangle className="w-4 h-4 shrink-0" />
                <span>
                  {isHi
                    ? `खोजे गए: ${unsyncedMessages.length} पुराने संदेश जो सर्वर पर नहीं हैं!`
                    : `Detected: ${unsyncedMessages.length} previous messages missing on server!`}
                </span>
              </h4>
              <p className="text-[11px] sm:text-xs text-slate-300 leading-relaxed">
                {isHi
                  ? 'आपके ब्राउज़र के बैकअप में कुछ पिछले संदेश मिले हैं जो सर्वर डेटाबेस रीसेट के कारण वर्तमान में सर्वर पर नहीं हैं। आप उन्हें रीस्टोर कर सकते हैं ताकि वे अन्य सभी निवेशकों को भी दिखाई दें।'
                  : 'We found past messages stored in your browser backup that are currently missing on the server. You can restore them so they become visible to all active users.'}
              </p>
            </div>
            <button
              type="button"
              ...
              className="w-full sm:w-auto shrink-0 px-4 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-black flex items-center justify-center gap-1.5 transition-all shadow-md shadow-amber-500/10 disabled:opacity-50 cursor-pointer"
            >
              {isSyncingHistory ? (
                <>
                ...
                <>
                  ...
                  <span>{isHi ? 'इतिहास रीस्टोर और सिंक करें' : 'Restore & Sync History'}</span>
                </>
              )}
            </button>
          </div>
        ...
        {localMessages.length === 0 ? (
          ...
          <div className="space-y-3">
            {dedupe(localMessages).map((msg, idx) => {
              ...
              return (
                ...
                >
                  <div className="space-y-1 flex-1">
                    ...
                    {msg.contentHi && msg.contentHi !== msg.content && (
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 self-end md:self-center flex-wrap">
                    <button
                      onClick={() => handleResendDirect(msg)}
                      disabled={isSubmitting}
                      className="px-2.5 py-1.5 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border border-amber-500/30 text-xs font-bold flex items-center gap-1 cursor-pointer transition-colors disabled:opacity-50"
                      title={isHi ? 'वही संदेश तुरंत दोबारा भेजें' : 'Re-send exact same message'}
                    >
                      <RotateCcw className="w-3.5 h-3.5 text-amber-400" />
                      <span>{isHi ? 'दोबारा भेजें' : 'Re-send'}</span>
                    </button>

                    <button
                      onClick={() => handleEditAndResend(msg)}
                      className="px-2.5 py-1.5 rounded-lg bg-teal-500/15 hover:bg-teal-500/25 text-teal-300 border border-teal-500/30 text-xs font-bold flex items-center gap-1 cursor-pointer transition-colors"
                      title={isHi ? 'एडिट करके दोबारा भेजें' : 'Edit & Re-send message'}
                    >
                      <Edit3 className="w-3.5 h-3.5 text-teal-400" />
                      <span>{isHi ? 'एडिट करें' : 'Edit'}</span>
                    </button>

                    <button
                      onClick={() => handleDelete(msg.id)}
                      className="px-2.5 py-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 text-xs font-bold flex items-center gap-1 cursor-pointer transition-colors"
